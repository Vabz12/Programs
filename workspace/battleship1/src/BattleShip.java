...
public class BattleShip extends JFrame {
	...
	private JMenuItem newGame, newServerGame, newClientGame, help, quit, about, feel, restart, internet;
	...
	public BattleShip() {
		...
		(fileMenu = new JMenu("File")).setMnemonic('F'); // This is the File
															// Menu
		...
		(optionMenu = new JMenu("Options")).setMnemonic('O'); // This is the
																// Console Menu
	...
	private class ItemHandler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == newGame) {
				...
				// battleWindow = new JInternalFrame("BattleShip Stand Alone
				// Mode: " + level);
				// battleWindow.getContentPane().add(new
				// GameStandAlone(difficulty));
				// desktop.add(battleWindow);
				isServer = false;
				newServerGame.setEnabled(false);
				newClientGame.setEnabled(false);
			...
			if (e.getSource() == about) {
				JOptionPane.showMessageDialog(c,
						"BattleShip Game\n\n" + "Program Design:        shashank pandya\n"
								+ "Board Design:          shashank pandya\n"
			...
			if (e.getSource() == internet) {
				try {
					gamePort = Integer.parseInt(
							JOptionPane.showInputDialog(c.getParent(), "Enter the port to use for game communication:",
									"Internet Settings", JOptionPane.QUESTION_MESSAGE));
			...
			if (e.getSource() == feel) {
				...
				for (int i = 0; i < looks.length; i++)
					options[i] = looks[i].getClassName();

				Object selection = JOptionPane.showInputDialog(c, "Choose a Look and Feel:", "Look and Feel",
				...
				if (selection != null) {
					try {
						...
						JOptionPane.showMessageDialog(c.getParent(), lnfe.getMessage(), "Look&Feel Exception",
